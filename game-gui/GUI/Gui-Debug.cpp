...
GUI_DEBUG::GUI_DEBUG() {
	#ifdef DEBUG
	std::cout<<"Debug mode active GUI_DEBUG\n";
	#endif
}
...
void GUI_DEBUG::showVec(std::string vecName, glm::vec3 vec, bool* open) {
...
void GUI_DEBUG::showVec(std::string vecName, glm::vec4 vec, bool* open)
{
	ImGuiWindowFlags flags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse;
...
void GUI_DEBUG::showTwoValues(std::string title, float val1, float val2, std::string nameVal1,
...
void GUI_DEBUG::showSingleVal(std::string valName, float& val, bool* open) {
...
void GUI_DEBUG::useSliderForVec2(std::string name, glm::vec2& vec, bool* open) {
...
void GUI_DEBUG::useSliderForFloat(std::string valName, float& val, bool* open) {
	ImGuiWindowFlags flags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse;
...
void GUI_DEBUG::enableBool(std::string name, bool& value, bool* open)
{
	ImGuiWindowFlags flags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse;
...
void GUI_DEBUG::enableTileMapEditing(bool& enable, bool& enableGrid, bool* open)
{
	ImGuiWindowFlags flags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse;
...
void GUI_DEBUG::showBoolVar(std::string valName, bool& val, bool* open)
{
	ImGuiWindowFlags flags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse;
...
void GUI_DEBUG::m_DebugPlayer(Player& player)
{
	...
	if (showHitTimer) this->showSingleVal("Player hit timer", player.m_hitTime, &showHitTimer);
	if (showColor) this->showVec("Player color", player.m_color, &showColor);
...
void GUI_DEBUG::m_spawnMobs(glm::vec3 pos)
{
