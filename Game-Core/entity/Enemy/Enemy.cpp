...
/*
 maxLeftPos and maxRightPos in the constructor are the max offset in the left and right of how 
 the enemy can move depending the initial pos, if are not defined, with the first thing that collide 
 will change the direction in the x axis
*/

EnemySlime2D::EnemySlime2D(int* renderIndex, float texSlot, glm::vec3 position,
	glm::vec3 maxLeftPos, glm::vec3 maxRightPos, glm::vec2 texturePosition,
	glm::vec2 textureSize, glm::vec2 size,
	glm::vec2 posOffset, glm::vec2 sizeOffset,
	glm::vec4 color)
	:Entity2D_Instaciaded(renderIndex,position,size,color,posOffset,sizeOffset
		,texSlot,texturePosition,textureSize), m_life(0),m_maxLeftPos(maxLeftPos),
	m_maxRightPos(maxRightPos),m_damageTime(1.3f),m_damageTimer(0.0f)
{
...
Rect EnemySlime2D::m_getEntityRect()
{
	if (this->m_direction.x == 1)
		return Rect(glm::vec2(this->m_position.x, this->m_position.y), glm::vec2(this->m_size.x, this->m_size.y),
			glm::vec2(this->m_previusPos.x, this->m_previusPos.y), this->m_posOffsetRect, this->m_sizeOffsetRect);
...
void EnemySlime2D::move(float& dt)
{
	...
	if (this->m_position.x > this->m_maxRightPos.x && this->m_maxRightPos.x > this->m_maxLeftPos.x)
		this->m_direction.x = -1;
	else if (this->m_position.x < this->m_maxLeftPos.x && this->m_maxLeftPos.x < this->m_maxRightPos.x)
		this->m_direction.x = 1;
...
void EnemySlime2D::m_onCollision()
{
	...
	{
		this->m_top = false;
		LOG(this->m_tagName + std::to_string(this->m_renderInstanceIndex) + " was hurt");
