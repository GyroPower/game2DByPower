...
Entity2D::Entity2D(float texSlot, glm::vec4 color, glm::vec3 position,
                   glm::vec2 size)
  ...
  this->vertexs.emplace_back(
      Vertex(glm::vec3(this->position.x, this->position.y + this->size.y, 0.0f),
             glm::vec2(0.0f), this->color, texSlot));

  // bottom right
  this->vertexs.emplace_back(
      Vertex(glm::vec3(this->position.x + this->size.x,
  ...
  // top left
  this->vertexs.emplace_back(Vertex(this->position, glm::vec2(0.0f, 1.0f),
                                    this->color, this->texSlot));
}

void Entity2D::updateVertexs() {
  this->vertexs[0].position =
      glm::vec3(this->position.x, this->position.y + this->size.y, 0.0f);
  this->vertexs[1].position = glm::vec3(this->position.x + this->size.x,
                                        this->position.y + this->size.y, 0.0f);
  this->vertexs[2].position =
      glm::vec3(this->position.x + this->size.x, this->position.y, 0.0f);
  this->vertexs[3].position =
...
// This is to have seperate the render and entity index, if a entity exists but
// is not redered well, the render index is set to other value, just to have
// track of the gpu memory and change it if the entity is visible or not and
// render it
static int entityIndex = 0;

Entity2D_Instaciaded::Entity2D_Instaciaded(int *renderIndex, glm::vec3 position,
                                           glm::vec2 size, glm::vec4 color,
                                           glm::vec2 posOffsetRect,
                                           glm::vec2 sizeOffsetRect,
                                           float texSlot, glm::vec2 texPos,
                                           glm::vec2 texSize)
    : m_color(color), m_position(position), m_size(size), texSlot(texSlot),
      m_entityIndex(entityIndex), m_texCoords(texPos, texSize),
      m_texOffset(glm::vec2(0.0f)), m_maxSpeed(120.0f), m_previusPos(position),
      m_grounded(false), m_speed(0.0f), m_wallTouch(false), m_pushed(false),
      m_right(false), m_left(false), state(IDLE),
      m_posOffsetRect(posOffsetRect), m_sizeOffsetRect(sizeOffsetRect),
      m_animTimeLimit(0.5f), m_animTime(0.0f), m_top(false), m_col(false),
      m_tagName(""), m_bottom(false), m_updateTimers(false),
      m_moveRoutine(false), m_delete(false), m_toDelete(false) {
...
void Entity2D_Instaciaded::move(float &dt) {
  /* Basic way of how should you entity move, can change if you want in a
   * children class */
...
void Entity2D_Instaciaded::setPosInterpolation(float &dt) {
  this->m_position =
      byPowerMath::lerp(this->m_previusPos, this->m_position, dt);
}

Rect Entity2D_Instaciaded::m_getEntityRect() {
  /*
          Basic way of how should you entity return the rect for collision
     detection, can change if you want in a children class
  */
  return Rect(this->m_position, this->m_size, this->m_previusPos);
}

std::string Entity2D_Instaciaded::m_getTagName() { return m_tagName; }
...
void Entity2D_Instaciaded::m_anim(float &dt) {
  /*
          set the logic of how should animate and update the texCoords on your
     children class
  */
}
...
Tile::Tile(int *renderIndex, glm::vec3 pos, glm::vec2 size, glm::vec4 color,
           bool visible)
...
bool Tile::m_isVisible() { return this->m_visible; }

void Tile::m_setRenderIndex(int index) { this->m_renderInstanceIndex = index; }
