...
Shader::Shader(const char *shaderVertexSource, const char *shaderFragmentSource,
  ...
  try {
    std::ifstream vShaderFileSource(shaderVertexSource),
        fShaderFileSource(shaderFragmentSource);
...
void Shader::checkCompileErrors(unsigned int object, std::string type) {
  ...
  if (type != "PROGRAM") {
    ...
    if (!success) {
      ...
      std::cout << "| ERROR::SHADER: " << this->shaderName
                << " Compile-time error: Type: " << type << "\n"
                << infoLog
                << "\n -- ---------------------------------------------- --"
                << std::endl;
    }
