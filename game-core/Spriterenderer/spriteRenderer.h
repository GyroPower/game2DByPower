...
/*
        this spriteRenderer class is for use batchRendering but I stoped to
   develop to use instance rendering
*/
...
public:
  ...
  void fillData(std::vector<Vertex> &vertices,
...
/*
        ...
        I wiil in the future change to be able to use OpengGL or Vulkan but that
   will be later,

        For those who doesn't know(mean beginers in programing and in c++),
        to handle polymorfism in vectors you would probably use a vector pointer
   of the base class to be able to use the virtual functions of the base class
   redefined in the children classes, that's why I use a vector pointer
   reference in the initFillData function (std::vector<baseClass*>& vectorName)
...
public:
  ...
  void
  initFillData(std::vector<std::unique_ptr<Entity2D_Instaciaded>> &entities);
  ...
  void scaleSingleEntity(Entity2D_Instaciaded *entity, float &scale,
                         Camera &cam);
