...
rendererDebugQuad::~rendererDebugQuad() { this->emptyData(); }
...
void rendererDebugQuad::reserveData(int sizeToReserve) {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 4, &vertices[0],
               GL_STATIC_DRAW);
...
void rendererDebugQuad::initData(
  ...
  if (entities.size() > 0) {
    ...
    glBufferSubData(GL_ARRAY_BUFFER, offsetColor,
                    (sizeof(glm::vec4) * entities.size()), &colors[0]);
    glBindBuffer(GL_ARRAY_BUFFER, this->m_VBO_pos);
    glBufferSubData(GL_ARRAY_BUFFER, offsetPos,
                    (sizeof(glm::mat4) * entities.size()), &modelMat4[0]);
  }

  this->m_instances = this->m_instances == 0
                          ? entities.size()
...
void rendererDebugQuad::draw(Camera &camera) {
  ...
  glBindVertexArray(this->m_VAO);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, this->m_IBO);
  glDrawElementsInstanced(GL_LINES, this->m_indices, GL_UNSIGNED_INT, 0,
                          this->m_instances);
